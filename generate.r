...
plot_rosetta = function (ts, z1, z2, up_to, rotations) {
    is = seq_len(up_to)
    z1 = z1[is]
    z2 = z2[is]
    ...
    pts1 = vapply(0 : rotations, f, complex(1L), t = ts[up_to], sign = 1)
    pts2 =  vapply(0 : rotations, f, complex(1L), t = ts[up_to], sign = -1)
...
z1data = f(ts, 1, rotations)
z2data = f(ts, -1, rotations)
...
for (frame in seq(frames)) {
    ...
    plot_rosetta(ts, z1data, z2data, up_to = idx, rotations = rotations)
    dev.off()
}
